function playVids(videoId) {
    ...
    if (vid.readyState > 3) {
        ...
        function trackLocation(e) {
            ...
            position = ((e.pageX - (bcr.x+15) ) / (bcr.width-15)).clamp(0.0, 1.0);
        }

        function trackLocationTouch(e) {
            // Normalize to [0, 1]
            var bcr = videoMerge.getBoundingClientRect();
            position = ((e.touches[0].pageX - (bcr.x+15)) / (bcr.width-15)).clamp(0.0, 1.0);
        }
        ...
        function drawLoop() {
            mergeContext.clearRect(0, 0, videoMerge.width, videoMerge.height); // Clear previous drawings
